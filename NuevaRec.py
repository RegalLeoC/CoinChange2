...
def CoinChange(coins, money, changeamount, amounts):
    missing = 0
    ...
    for i in range(len(coins)):
        if amounts[i] >= 0:
            if (coins[i] + missing) <= 0:
                ...
                newamounts = amounts[i] - 1
                if CoinChange(coins, money, newchangeamount, newamounts):
                    return True


def recCoin(coins, money, amounts):
    changeamount = [0]
    change = (CoinChange(coins, money, changeamount, amounts))
    return changeamount
